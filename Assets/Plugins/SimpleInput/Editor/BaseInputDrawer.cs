...
namespace SimpleInputNamespace {
  public abstract class BaseInputDrawer : PropertyDrawer {
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
      ...
      var inputRect = new Rect(position.x, position.y, position.width - 90, position.height);
      var valueRect = new Rect(position.x + position.width - 85, position.y, 85, position.height);
