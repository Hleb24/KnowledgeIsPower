    ...
    private void Awake() {
      if (axes.Length > 0) {
        ...
        for (var i = 0; i < axisInputs.Length; i++) {
          try {
            var unityAxis = new SimpleInput.AxisInput(axes[i]) { value = Input.GetAxisRaw(axes[i]) };
      ...
      if (buttons.Length > 0) {
        ...
        for (var i = 0; i < buttonInputs.Length; i++) {
          try {
            var unityButton = new SimpleInput.ButtonInput(buttons[i]) { value = Input.GetButton(buttons[i]) };
      ...
      if (mouseButtons.Length > 0) {
        ...
        var index = 0;
        for (var i = 0; i < mouseButtonInputs.Length; i++) {
          try {
            var unityMouseButton = new SimpleInput.MouseButtonInput(mouseButtons[i]) { value = Input.GetMouseButton(mouseButtons[i]) };
      ...
      if (keys.Length > 0) {
        ...
        for (var i = 0; i < keyInputs.Length; i++) {
          var unityKey = new SimpleInput.KeyInput(keys[i]) { value = Input.GetKey(keys[i]) };
