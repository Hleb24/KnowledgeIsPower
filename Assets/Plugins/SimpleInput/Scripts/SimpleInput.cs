...
public class SimpleInput : MonoBehaviour {
...
#endif
	/// <summary>
	///   When SimpleInput.GetAxisRaw's value is 0f and SimpleInput.GetAxis' value is within this range, SimpleInput.GetAxis'
	///   value will snap to 0f
	/// </summary>
	public static float GetAxisDeadZone = 0.025f;
	/// <summary>
	///   If set to true and the values of SimpleInput.GetAxis and SimpleInput.GetAxisRaw have different signs,
	///   SimpleInput.GetAxis will jump to 0f and continue from there
	/// </summary>
    ...
    set {
      if (m_trackUnityInput != value) {
        ...
        if (!m_trackUnityInput) {
          for (var i = 0; i < trackedUnityAxes.Count; i++) {
            trackedUnityAxes[i].ResetValue();
          }

          for (var i = 0; i < trackedUnityButtons.Count; i++) {
            trackedUnityButtons[i].ResetValue();
          }

          for (var i = 0; i < trackedUnityMouseButtons.Count; i++) {
  ...
  private static readonly Dictionary<string, Axis> axes = new();
  private static readonly List<Axis> axesList = new();

  private static readonly List<AxisInput> trackedUnityAxes = new();
  private static readonly List<AxisInput> trackedTemporaryAxes = new();

  private static readonly Dictionary<string, Button> buttons = new();
  private static readonly List<Button> buttonsList = new();

  private static readonly List<ButtonInput> trackedUnityButtons = new();
  private static readonly List<ButtonInput> trackedTemporaryButtons = new();

  private static readonly Dictionary<int, MouseButton> mouseButtons = new();
  private static readonly List<MouseButton> mouseButtonsList = new();

  private static readonly List<MouseButtonInput> trackedUnityMouseButtons = new();
  private static readonly List<MouseButtonInput> trackedTemporaryMouseButtons = new();

  private static readonly Dictionary<KeyCode, Key> keys = new();
  private static readonly List<Key> keysList = new();
  ...
  private static void OnSceneChanged(Scene scene, LoadSceneMode loadSceneMode) {
    for (var i = 0; i < trackedTemporaryAxes.Count; i++) {
      trackedTemporaryAxes[i].StopTracking();
    }

    for (var i = 0; i < trackedTemporaryButtons.Count; i++) {
      trackedTemporaryButtons[i].StopTracking();
    }

    for (var i = 0; i < trackedTemporaryMouseButtons.Count; i++) {
  ...
  private static void TrackAxis(string axis, bool trackUnityAxisOnly = false) {
    try {
      var unityAxis = new AxisInput(axis) { value = Input.GetAxisRaw(axis) };
      if (!m_trackUnityInput) {
      ...
      if (!trackUnityAxisOnly) {
        var temporaryAxis = new AxisInput(axis) { value = 0f };
  ...
  private static void TrackButton(string button, bool trackUnityButtonOnly = false) {
    try {
      var unityButton = new ButtonInput(button) { value = Input.GetButton(button) };
      if (!m_trackUnityInput) {
      ...
      if (!trackUnityButtonOnly) {
        var temporaryButton = new ButtonInput(button) { value = false };
  ...
  private static void TrackMouseButton(int button, bool trackUnityMouseButtonOnly = false) {
    try {
      var unityMouseButton = new MouseButtonInput(button) { value = Input.GetMouseButton(button) };
      if (!m_trackUnityInput) {
      ...
      if (!trackUnityMouseButtonOnly) {
        var temporaryMouseButton = new MouseButtonInput(button) { value = false };
  ...
  private void Update() {
    ...
    for (var i = 0; i < axesList.Count; i++) {
    ...
    for (var i = 0; i < mouseButtonsList.Count; i++) {
